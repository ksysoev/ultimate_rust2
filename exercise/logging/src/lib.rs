...
impl Frog {
    pub fn new() -> Self {
        ...
        debug!(target: "frogger::Frog::new", "A new Frog has been created");
        Default::default()
    }
    pub fn hop(&mut self) {
        self.energy -= 1;
        // 3. Use info!() to log that a Frog hopped, and how much energy is left
        info!(target: "frogger::Frog::hop", "A Frog hopped, and has {} energy left", self.energy);
        if self.energy == 0 {
            // 4. Use warn!() to warn that the frog will go to sleep since he ran out of energy
            warn!(target: "frogger::Frog::hop", "The frog will go to sleep since he ran out of energy");
            self.sleep();
        }
    }
    pub fn sleep(&mut self) {
        if self.sleeping {
            ...
            error!(target: "frogger::Frog::sleep", "The Frog is already asleep");
        } else {
            self.sleeping = true;
        }
    }
...
impl Default for Frog {
    fn default() -> Self {
        ...
        trace!(target: "frogger::Frog::default","A default value was generated");
        Frog {
            energy: 5,
            sleeping: false,
        }
